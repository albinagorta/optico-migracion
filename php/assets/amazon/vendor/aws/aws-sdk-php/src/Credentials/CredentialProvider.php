...
class CredentialProvider
{
    ...
    const RUTA_CREDENTIAL = 'credentials'; ///.aws/credentials
    ...
    {
        $filename = $filename ?: (self::getHomeDir() . self::RUTA_CREDENTIAL);
        ...
        return function () use ($profile, $filename, $config) {
            ...
            if (!is_readable($filename)) {
                return self::reject("Cannot read credentials from $filename"."|".self::getHomeDir() );
            }
    ...
    private static function loadProfiles($filename)
    {
        ...
        if ($filename === self::getHomeDir() . self::RUTA_CREDENTIAL
    ...
    private static function loadDefaultProfiles() {
        $profiles = [];
        $credFile = self::getHomeDir() . self::RUTA_CREDENTIAL;

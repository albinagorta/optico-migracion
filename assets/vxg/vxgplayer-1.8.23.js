window.vxgplayer = function(id, options){
	...
	if(!window.vxgplayer.players[id]){
		...
		window.vxgplayer.players[id] = new function(id, options){
			...
			var nmf_src = "assets/vxg/pnacl/Release/media_player.nmf";
			var url = "";
			...
			self.fullscreen = function(e){
				var idscre = e.path[2].id;
				console.log(e.path[2].id);
				
				console.log($("#"+idscre).width());
				console.log($("#"+idscre).height());

				if ($("#"+idscre).width() == 576) {
					$("#"+idscre).css({"width": "800px", "height": "500px"});
				} else {
					$("#"+idscre).css({"width": "580px", "height": "430px"});
				}
...
window.vxgplayer.checkWebSocket = function(){
	...
	setTimeout(function(){
		...
		// var	testWebSocket = new WebSocket('ws://10.1.2.2642:' + vxgplayer.webserverport + '/');
		console.log("ax");
...
function iniciovxg(e) {
	...
	//var els = document.getElementById("10.20.1.105");

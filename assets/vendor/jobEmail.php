...
where sh.nralerta=2 AND sh.in_enviamail='1'
...
if (!empty($newRESULT)) {
    foreach ($newRESULT as $key => $row) {
        ...
        $correoCliente = 'comunicacionesadmins@minedu.gob.pe';
        $tipoSensor = $row['class_id'];
        $ip = $row['hostname'];
        $id_servicio = $row['id_servicio'];
        $id_tiposerv = $row['id_tiposerv'];
        ...
        if  ($tipoSensor == "oct-in" || $tipoSensor == "oct-out") {        
         ...
         try {
            ...
            // Set a "subject"
            $message->setSubject('Alerta Sensor. '.$id_servicio." ".$tipoSensor);
            ...
            $message->addCc('framos@opticaln.onmicrosoft.com', 'Fernando Ramos');
